...
const toParseSchema = (schema) => {
  for (const key in schema.fields) {
    if (typeof schema.fields[key] === 'string') {
      schema.fields[key] = JSON.parse(schema.fields[key]);
    }
  }
...
const buildWhereClause = ({ schema, query, index }) => {
  ...
  for (const fieldName in query) {
    ...
    if (fieldName.indexOf('.') >= 0) {
      ...
      if (fieldValue === null) {
        patterns.push(`JSON_VALUE(\`${components[0]}\`, ${name}) IS NULL`);
      } else {
        patterns.push(`JSON_CONTAINS(\`${components[0]}\`, '${fieldValue}', '${name}')`);
      }
