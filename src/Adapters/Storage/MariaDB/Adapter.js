...
const debug = function(){
  let args = [...arguments];
  args = ['MariaDB: ' + arguments[0]].concat(args.slice(1, args.length));
...
export class Adapter {
  ...
  _ensureSchemaCollectionExists() {
    ...
    return this.connect()
      .then(() => this.database.query(`CREATE TABLE IF NOT EXISTS \`_SCHEMA\` (\`className\` ${defaultUniqueKeyLength}, \`schema\` TEXT, \`isParseClass\` BOOL, PRIMARY KEY (\`className\`), CHECK (\`schema\` IS NULL OR JSON_VALID(\`schema\`)));`));
  }
  ...
  setClassLevelPermissions(className, CLPs) {
    const qs = "UPDATE `_SCHEMA` SET `$2:name` = JSON_SET(COALESCE(`$2:name`, '{}'), '$.$3:name', '$4:name')) WHERE `className` = '$1:name'";
  ...
  createTable(className, schema) {
    ...
    Object.keys(fields).forEach((fieldName) => {
      ...
      if (parseType.type === 'Object' || parseType.type === 'Array' || parseType.type === 'Bytes' || ['_rperm', '_wperm'].indexOf(fieldName) >= 0) {
        patternsArray.push(`\`$${index}:name\` TEXT, CHECK (\`$${index}:name\` IS NULL OR JSON_VALID(\`$${index}:name\`))`);
        index += 1;
        return;
      }
  ...
  addFieldIfNotExists(className, fieldName, type) {
    ...
    return this.connect()
      ...
      .then(() => {
        return this.database.query('SELECT `schema` FROM `_SCHEMA` WHERE `className` = \'$1:name\' and JSON_VALUE(`schema`,\'$.fields.$2:name\') is not null', [className, fieldName]);
      }).then(([result]) => {
        if (result[0]) {
  ...
  deleteFields(className, schema, fieldNames) {
    ...
    return this.connect()
      .then(() => this.database.query("UPDATE `_SCHEMA` SET `schema` = JSON_SET(COALESCE(`schema`, '{}'), '$.fields', '$1:name')) WHERE `className`='$2:name'", [JSON.stringify(schema.fields), className]))
      .then(() => {
        if (values.length > 1) {
  ...
  getAllClasses() {
    ...
    return this._ensureSchemaCollectionExists()
      ...
      .then(([rows]) => {
        return rows.map((row) => {
          return toParseSchema({ className: row.className, ...(JSON.parse(row.schema)) });
        });
      }).catch((error) => {
        console.log('get all class error');
        console.log(error);
      });
  }
  ...
  getClass(className) {
    ...
    return this.connect()
      .then(() => this.database.query('SELECT * FROM `_SCHEMA` WHERE `className` COLLATE utf8_general_ci =\'$1:name\'', [className ]))
      .then(([result]) => {
        if (result.length === 1) {
    ...
    for (const fieldName in update) {
      ...
      if (fieldValue === null) {
        ...
        const generate = (jsonb, key, value) => {
          return `JSON_SET(COALESCE(\`${fieldName}\`, '{}'), '$.${key}', '${value}'))`;
        }
  ...
  find(className, schema, query, { skip, limit, sort, keys }) {
    ...
    return this.connect()
      ...
      .then(([results]) => results.map(object => {
        ...
        // if (object._rperm) {
        //   console.log(object._rperm[0]);
        //   if (object._rperm.contains('*')) {
        //     object._rperm.replace('*','');
        //     object._rperm = JSON.parse(object._rperm);
        //     console.log(object._rperm);
        //   } else {
        //     object._rperm = JSON.parse(object._rperm);
        //   }
        // }
        // if (object._wperm) {
        //   object._wperm = JSON.parse(object._wperm);
        // }
